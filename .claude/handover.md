...
| **HavunClub van de server af (18-07)** | Hele `/var/www/havunclub` (391 MB) + beide DB's + MySQL-user + cert + nginx-vhosts verwijderd. Backup: `/root/backups/havunclub-2026-07-18/` (DB-dumps + vhost-configs). Code blijft op PC + GitHub. De `aeterna-latest.apk` was géén verlies — identieke canonieke kopie staat op `havuncore/webapp/public/downloads/aeterna/`. **Ruimtewinst marginaal** (disk 73%→72%, 0 RAM: geen eigen fpm-pool) |
