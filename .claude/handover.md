...
**Ook 2 juli — Web-push backend voor kritieke health-alerts (commit 2acd428):** Henks keuze (PWA-push i.p.v. mail) om het notificatie-gat te dichten. Backend gebouwd + getest (1272 groen), hergebruikt SafeHavuns patroon: `minishlink/web-push`, VAPID-keys in de Vault (at-runtime gelezen), `push_subscriptions` + `PushController` (subscribe/unsubscribe/vapid-public-key), `vapid:setup`-command, en een push-hook in `HealthAlertCommand` bij verse `down`+`critical`. **Nog te doen (prod = Henks go, zie `docs/kb/plans/health-alerts-webpush-blueprint.md`):** deploy + `migrate` + `vapid:setup` + nginx-allowlist `/api/push/*` op de server; en het **frontend-deel** (service-worker push + subscribe-knop bij de 🔔 bel) in een **havuncore-webapp-sessie**.
