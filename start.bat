...
if not exist "%CODEDIR%\vendor\" (
    echo [INFO] Dependencies installeren...
    cd /d "%CODEDIR%"
    "%DRIVE%\tools\php\php.exe" "%DRIVE%\tools\php\composer.phar" install --no-dev
)
echo [INFO] Server starten op http://localhost:8000 in nieuw venster...
start "%PROJNAME% - Server" cmd /k "cd /d "%CODEDIR%" && set PATH=%DRIVE%\tools\php;%DRIVE%\tools\git\cmd;%PATH% && title %PROJNAME% - localhost:8000 && "%DRIVE%\tools\php\php.exe" artisan serve"
echo [OK] Server gestart (stop via CTRL+C in dat venster)
goto :action_menu
...
:terminal
echo [INFO] Terminal openen in nieuw venster...
start "Havun Dev Terminal" cmd /k "set PATH=%DRIVE%\tools\php;%DRIVE%\tools\git\cmd;%DRIVE%\tools\git\usr\bin;%DRIVE%\tools\nodejs;%DRIVE%\tools\npm-global;%DRIVE%\tools\7-Zip;%PATH% && doskey composer=%DRIVE%\tools\php\php.exe %DRIVE%\tools\php\composer.phar $* && cd /d %DRIVE%\projects && title Havun Dev"
echo [OK] Terminal geopend
goto :project_menu
