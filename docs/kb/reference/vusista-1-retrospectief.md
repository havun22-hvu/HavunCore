---
title: Wat er in Vusista fout ging — bericht aan HavunCore
type: reference
scope: havuncore
last_updated: 2026-07-30
---

# Wat er in Vusista fout ging, en hoe HavunCore dat voortaan voorkomt
...
## Concrete taak voor HavunCore, en wat Vusista2 zelf doet

**Vusista2 richt zichzelf in.** Niet uit eigenzinnigheid: `project:scaffold vusista2` zou nu
een Laravel-opzet met staging, productie en een deploy-pipeline neerzetten voor een
Rust-desktopapp die geen van die dingen heeft. Dat is exact de fout hierboven, nog een keer.
De inrichting gebeurt dus handmatig en toegesneden: CLAUDE.md voor een Rust-project,
docs-structuur, en de product-docs uit Vusista 1 die onveranderd geldig blijven.

**HavunCore krijgt daarmee een sjabloon in plaats van een klacht.** De taak, in volgorde:

1. **`project:scaffold` krijgt een app-type** (`--type=web|desktop|api|mobile`) en vraagt
   ernaar als het ontbreekt. Bij `desktop`: geen webserver, geen staging, geen
   deploy-pipeline, geen `.github/workflows/deploy-*.yml`.
2. **Neem Vusista2's inrichting als sjabloon voor `desktop`** zodra die staat — dan is het
   een gewerkt voorbeeld en geen bedacht formaat.
3. **Voeg de vijf intakevragen toe** (zie boven) als verplichte stap vóór het scaffolden, en
   leg de antwoorden vast in het project zelf. Dan staat er zwart-op-wit onder welke aannames
   de stack gekozen is — en dat is precies wat bij Vusista 1 ontbrak.
4. **Zet Vusista in `config/quality-safety.php`.** Het staat er niet in, dus er heeft nooit
   een geplande V&K-scan op gedraaid: geen dagelijkse `composer audit`, geen secrets-scan,
   geen session-check. Ontdekt op 30 juli, en het gold vier maanden lang.
5. **Ruim de Vusista-serveromgeving op.** `/var/www/vusista/{production,staging}` hoort er
   niet te zijn voor een desktop-app; de staging-deploy faalde dertien dagen ongemerkt en de
   demo serveert een 500. Zie de handover van Vusista 1 voor de details.
