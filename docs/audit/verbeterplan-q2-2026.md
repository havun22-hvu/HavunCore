...
| VP-18 | Alpine.js CSP migratie (JudoToernooi) | IN-PROGRESS — feature branch `feat/vp18-alpine-csp-migration` (JT repo). @alpinejs/csp geïnstalleerd, @alpinejs/collapse blijft. Nieuwe `resources/js/alpine-components.js` met 24 generieke componenten. **57 van 61 inline x-data directives** gemigreerd + alle bijbehorende @click-assignments naar method calls. Vite build groen. Resterend: 4 complexe inline (setTimeout-arrows, $watch-arrows, parent-scope refs) + 31 function-call `x-data="fn()"` refs (clubsPage/judokaTable/matInterface/etc.) met inline `<script>` definities — vergelijkbaar aan HP batch 4/5 | Medium | 31-07-2026 | Claude (Mozilla Observatory) |
