...
| VP-16 | Kwartaalse mutation testing | READY-FOR-FIRST-RUN — Infection PHP geïnstalleerd (composer dev-dep), `infection.json5` config met scope=16 payment-classes, filter aangescherpt naar ~500 payment-relevante tests (sluit Arweave-crypto unit tests + verify_bunq_payments command tests uit). GitHub Action workflow `mutation-test.yml` met cron (1e jan/apr/jul/okt 03:00 UTC) + manuele trigger. Eerste baseline-meting komt automatisch uit Q3-cron op 1 juli 2026 | Medium | 30-09-2026 | Claude (v3.0 contra-review) |
...
| VP-18 | Alpine.js CSP migratie (JudoToernooi) | FEATURE-BRANCH-READY — 22 batches in `feat/vp18-alpine-csp-migration`. Runbook `docs/kb/runbooks/alpine-csp-migratie.md` (9 conversie-patronen, herbruikbaar voor toekomstige Havun projecten). Alle 17 aktieve views omgezet: deviceToegangen, helpPage, pouleSelect+noodplanPrint, milestonesPage, clubsPage, wimpelPage, judokaDetail, weeglijstTable (+countdown), wedstrijddagPoules (heartbeatToggle), templateManager, stambestandPage, weeglijst-live-admin, liveBackup+networkConfig, wimpelPuntenLog, coach judokas, judokaTable, mobielApp (4-tab), sprekerInterface, matInterface + bracketTabs, publiekApp (outer), editTabs + templateSave. Nieuwe shared components: bracketTabs, editTabs, rotate90Class op toggle, openAbout/closeAbout op modalWithAbout, selectFilter+filterButtonClass op filterSwitch, matchesFilter+notEditing op inlineEdit. Offline/index.blade.php bewust overgeslagen (eigen CDN-alpine). Vite build groen, safety tag `vp18-pre-fullmigration` voor rollback. **Laatste stap**: `'unsafe-eval'` uit CSP middleware verwijderen + end-to-end smoke test in browser voor edge cases (publiek-view, mat interface complex brackets). Geschatte restscope: 2-3 uur smoke test + bugfix | Medium | 31-07-2026 | Claude (Mozilla Observatory) |
