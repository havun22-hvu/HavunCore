...
| VP-11 | Alpine.js CSP migratie (Herdenkingsportaal) | PARTIAL — feature branch `feat/vp11-alpine-csp-migration` (HP repo). Klaar: alle 17 inline `x-data` → `Alpine.data()` componenten (toggle, dismissible, autoDismiss, navMenu, mobileNav, navWithColorPicker, cookieBanner, memorialTabs, lightbox, uploadForm, profileSection). Vite build groen. Resterend: 8 inline @click-assignments + 3 function-call `x-data="..()"` in `memorials/show.blade.php` (memorialGallery/condolenceManager/photoUploader staan als inline `<script>` in view) + compound x-show expressies. Branch wacht op staging-test voor merge | Medium | 30-06-2026 | Claude |
