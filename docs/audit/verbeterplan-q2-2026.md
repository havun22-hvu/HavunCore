...
| VP-18 | Alpine.js CSP migratie (JudoToernooi) | IN-PROGRESS — feature branch `feat/vp18-alpine-csp-migration` (JT repo). @alpinejs/csp geïnstalleerd + @alpinejs/collapse blijft. Nieuwe `resources/js/alpine-components.js` met 26 generieke componenten. **60 van 61 inline x-data directives** gemigreerd + alle @click-assignments naar method calls. **1 van 31 function-call `x-data="fn()"` refs extracted** (internetIndicator). Vite build groen. Resterend: 30 function-call refs (clubsPage, judokaTable, matInterface, publiekApp, etc.) + 1 complex activeFavoriet met $watch-arrow — elk vereist Alpine.data() extractie met data-* attributes voor Blade-data (vergelijkbaar aan HP batch 4/5). Geschatte vervolgwerk: 5-8 uur | Medium | 31-07-2026 | Claude (Mozilla Observatory) |
