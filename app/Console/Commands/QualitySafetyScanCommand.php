...
class QualitySafetyScanCommand extends Command
{
    protected $signature = 'qv:scan
        {--only= : Run only one check (composer|npm|ssl|observatory|server|forms|ratelimit|secrets|session-cookies)}
        {--project= : Scan only one project (slug)}
        {--json : Emit machine-readable JSON on stdout}';

    protected $description = 'Cross-project Quality & Safety scan (composer audit, npm audit, SSL expiry, Mozilla Observatory, server health, form-validation coverage, rate-limit coverage, hardcoded-secrets detection, session-cookie flags)';
    ...
    public function handle(QualitySafetyScanner $scanner): int
    {
        ...
        $availableChecks = ['composer', 'npm', 'ssl', 'observatory', 'server', 'forms', 'ratelimit', 'secrets', 'session-cookies'];
